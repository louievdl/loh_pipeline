...
# +-- hla-polysolver
# |
# +-- polysolver_inst (generated by hla-polysolver build process)
# |
# +-- hla_fasta (fasta file of hla loci and related files)
# |
# +-- genome (genome fasta, GC-content wiggle files, exon and HLA region bedfiles)
# |
# +-- LOHHLA
...
mkdir $PPLN_BASE_DIR
cd $PPLN_BASE_DIR
git clone https://github.com/louievdl/loh_pipeline.git
...
cd $PPLN_BASE_DIR/hla-polysolver/
SRC_DIR=$PPLN_BASE_DIR/hla-polysolver && PREFIX=$PPLN_BASE_DIR/polysolver_inst && mkdir $PREFIX
source build.sh # put tools in right places and let polysolver know where it is; finishes in $PPLN_BASE_DIR/hla-polysolver/include/strelka-upstream-v1.0.11
cd $PPLN_BASE_DIR
mkdir $PPLN_BASE_DIR/hla_fasta
cp -a $PPLN_BASE_DIR/hla-polysolver/data/abc_complete.fasta $PPLN_BASE_DIR/hla_fasta
# novoalign_header.sam is generated from abc_complete.fasta

# get updated lohhla
cd $PPLN_BASE_DIR
git clone https://github.com/louievdl/LOHHLA.git
cp -a $PPLN_BASE_DIR/LOHHLA/data/hla.dat $PPLN_BASE_DIR/hla_fasta
...
bash Anaconda3-2020.02-Linux-x86_64.sh # installed in $PPLN_BASE_DIR/anaconda3
